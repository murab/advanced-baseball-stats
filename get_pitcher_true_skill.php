...
foreach ($KpercentMinusXwoba as $key => $player) {
    ...
    echo "{$player['rank_formatted']} | {$player['name_formatted']} | {$player['k_percentage_formatted']}% | {$player['velo_formatted']} mph\n";
...
foreach ($custom_lists as $list) {
    ...
    foreach ($custom_players[$list] as $pitcher) {
        echo "{$pitcher['rank_formatted']} | {$pitcher['name_formatted']} | {$pitcher['k_percentage_formatted']}% | {$pitcher['velo_formatted']} mph\n";
    }
}
